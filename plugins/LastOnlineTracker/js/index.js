      ...
      children: [
        ...
        jsxs(revenge.discord.design.Design.TableRowGroup, {
          ...
          children: [
            ...
            jsx(revenge.discord.design.Design.TableSwitchRow, {
              label: "Member list", subLabel: "Server and DM member lists both",
...
const combine = (native, label) =>
  hasVisibleText(native)
    ? jsxs(revenge.react.ReactNative.View, { style: { flexDirection: "row", alignItems: "center", flexWrap: "wrap" }, children: [native, text(` · ${label}`)] })
    : text(label);
        ...
        cleanup(
          revenge.patcher.instead(loc.parent, loc.key, (args, orig) => {
            ...
            if (actNode && activityStatusOrig) {
              actNode.type = (p) => combine(activityStatusOrig(p), label);
              return rendered;
            }

            const nameNode = revenge.utils.tree.findInTree(rendered, (n) => n?.props?.children?.[0]?.props?.variant === "text-md/medium", WALK);
            if (nameNode) {
              const original = nameNode.props.children;
              nameNode.props.children = jsxs(revenge.react.ReactNative.View, {
                style: { flexDirection: "column" },
                children: [jsx(revenge.react.ReactNative.View, { style: { flexDirection: "row", alignItems: "center" }, children: original }), text(label)]
              });
            }
            return rendered;
          })
